...
\begin{itemize}
    \item on the one side, CS papers about the form, and the productivity of source code. there is focus on form, but not so much about aesthetics per se (no over-arching theory, and references to literature, craft, architecture and maths)
    \item as a subset, include technical literature? things like clean code, code complete, etc. (these put more emphasis on how code is supposed to look like)
    ...
    \item finish on the books that have the most details: beutiful code, aesthetics of code phd theses, expressive code
\end{itemize}
