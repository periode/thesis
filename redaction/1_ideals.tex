...
In another paper published by the ACM, programming's leading academic publisher, Kristiina Karvonen argues for simplicity not just as a design goal, as leveraged by human-computer interface designers, but as a term with a longer history within the tradition of aesthetic philosophy, especially the work of Johann Joachim Winckelmann\cite{karvonen_beauty_2000}. In particular, she stresses the difficulty "to create significant, that is beautiful works of art with simple means"\cite{karvonen_beauty_2000}. Here, her correlation between significance and beauty hints at the semantic role of simplicity, as a means to communicate ideas (i.e. \emph{signify}) to an audience.

A final remark on simplicity and programming regarding the communication of ideas is hinted at by Richard P. Gabriel in his use of the concept of \emph{compression} in both poetry and programming. In an interview with Janice J. Jeiss, he states:
...
This answer by Mason Wheeler, posted on the software engineering \emph{Stack Exchange} forum, in response to the question "How can you explain "beautiful code" to a non-programmer?"\footnote{\url{https://softwareengineering.stackexchange.com/questions/65216/how-can-you-explain-beautiful-code-to-a-non-programmer}}, highlights simplicity's opposite, cleverness.
...
This strive towards attaining an inverse relationship between the complexity of an idea and the means to express it evokes another related criteria for beautiful source code: elegance.

% perrin -> definitions by negations, brief, concise, etc.

% Macneil: "coding problems are about conceptualization, so beautiful code represents a remarkable conceptualization of the problem", and a beautiful execution. (programming really is just the craft of ideas). "for instance, when a problem can be reduced to an existing problem, it gives us an insight into the nature of the problem itself." (and elegance uses simple trucks to achieve difficult tasks). he concludes with `quicksort`. -> another concrete example of quicksort, with a hint at the craft of ideas

% "Elegance combines simplicity, efficiency, and brilliance, and produces a feeling of pride. Elegance is when you replace a procedure with a table, or realize that you can use recursion—which is almost always elegant" -> focus on a specific device in progrmaming for elegance

% *'There is no such thing as elegance or subtlety in the way I use software.'* Belgrade-based sound artist Svetlana Maraš

% efatmaneshik, ryan - __elegance has both quantitative/practical and qualitative/abstract/romantic aspects__
...
% clear
\begin{lstlisting} from https://dzone.com/articles/15-ways-to-write-beautiful-code
  void SomeMethod(){
    if(x != y){
        //-- stuff
    }
...
% add qualifiers from artists, hackers: e.g. mind-bending, etc.

Negative words (1000)

\begin{itemize}
    \item smelly
    \item entangled, spaghetti, interdependency (chandra)
    \item spaghetti
    \item bug
    \item verbose
\end{itemize}
...
- simplicity
  ...
  % and then we can look at specifically this one to highlight all the different analogies: https://softwareengineering.stackexchange.com/questions/65216/how-can-you-explain-beautiful-code-to-a-non-programmer
  ...
  % conclusion: it is very coherent, people know what they're talking about, it's a thing. what kind of thing it is, then, depends on what kind of register we're mobilizing.
...
% mclennan talking about elegance has a nice transition as it allows for the development of two parallels: elegance in engineering and elegance in mathematics.
